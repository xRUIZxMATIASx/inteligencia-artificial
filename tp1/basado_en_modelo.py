...
if __name__ == "__main__":
    ...
    floor = create_floor()
    position = 0#random.randint(0, len(floor))
    false_floor = create_false_floor(len(floor), position)
    direction = directions[1]#directions[random.randint(0, 1)]
    total_moviments = 0
    movement_changes = 0
    ...
    while True:
        ...
        if history_of_positions[position_on_the_ground] < 3:
        ...
        if keep_cleaning:
            ...
            state = check_floor_state(floor, position)
            print("State:", floor[position])
            print("The floor has been cleaned")
            break
